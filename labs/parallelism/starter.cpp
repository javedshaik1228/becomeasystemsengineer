...
std::uint64_t parallel_sum(std::uint64_t item_count, unsigned thread_count) {
  ...
  // TODO 1: clamp the worker count and partition [0, item_count) into balanced,
  // non-overlapping ranges. Handle item_count == 0 without launching a thread.
  // TODO 2: run one range per std::jthread, write one private partial, join every
  // worker, and combine only after the joins establish visibility.
  (void)item_count;
  throw std::logic_error{"implement parallel_sum"};
}
